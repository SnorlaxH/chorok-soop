...
export const isVodPage = () => {
    return document.URL.includes("vod.sooplive.com") || document.URL.includes('vod.afreecatv.com');
}
...
export const injectVodPage = async () => {
    ...
    const injectInterval = setInterval(async () => {
        const elNextVideo = await waitingElement('.nextvideo')
        if (elNextVideo?.style.display !== 'block') {
            inject();
